...
def create_table(courseName, tableName, dataPath, overwrite = False):
  if tableName == "ht_daily_metrics":
    df = spark.read \
      .csv(dataPath, header = True) \
      .select(
        "device_id",
        "dte",
  ...
  try:
    ...
    if overwrite:
      df.write.format("delta").mode("overwrite").option("mergeSchema", "true").save(f"/{courseName}/{tableName}")
    elif not overwrite:
      df.write.format("delta").option("mergeSchema", "true").save(f"/{courseName}/{tableName}")
  spark.sql(
...
# MAGIC %fs rm -r /dsfda/ht-users-non-athletes

# COMMAND ----------

# MAGIC %fs rm -r /dsfda/ht-users-sedentary

# COMMAND ----------

# MAGIC %sql
# MAGIC DROP TABLE IF EXISTS dsfda.ht_users_non_athletes;
# MAGIC DROP TABLE IF EXISTS dsfda.ht_users_sedentary;

# COMMAND ----------

spark.sql(
    """
    CREATE OR REPLACE TEMPORARY VIEW ht_users_with_metrics AS
    (
      SELECT
        ht_users.device_id,
        dte,
        ...
        ht_users.lifestyle
      FROM
        ht_users JOIN ht_daily_metrics
        ON ht_users.device_id = ht_daily_metrics.device_id
    )
"""
);
...
spark.sql(
    """
    DROP TABLE IF EXISTS ht_agg;
"""
)

spark.sql(
    """
    CREATE TABLE ht_agg AS
    (
      SELECT
      device_id,
      MEAN(bmi) AS mean_bmi,
      MEAN(active_heartrate) AS mean_active_heartrate,
      MEAN(resting_heartrate) AS mean_resting_heartrate,
      MEAN(vo2) AS mean_vo2,
      MEAN(steps) AS mean_steps,
      lifestyle
      FROM ht_users_with_metrics
